...
const initWeekData = () => dayOrder.map((day) => ({ name: day, appointments: 0 }));
const initMonthWeekData = () =>
	Array(4)
		.fill(0)
		.map((_, i) => ({ name: `Week ${i + 1}`, appointments: 0 }));
const initYearMonthData = () => monthOrder.map((month) => ({ name: month, appointments: 0 }));
...
const adminController = {
	...
	addEmployee: async (req, res) => {
      ...
      `;
			...
			const values_auth = [employeeId.Employee_ID, hashedPassword, new Date(), new Date()];
	...
	getAllStats: async (req, res) => {
		...
		try {
			...
			const [appointments] = await sequelize.query("SELECT Date_Time, Department_ID FROM Appointments", {
				transaction: t,
			});

			const [patients] = await sequelize.query("SELECT Gender FROM Patients", { transaction: t });

			const [departments] = await sequelize.query("SELECT Department_ID, Department_Name FROM Departments", {
				transaction: t,
			});
