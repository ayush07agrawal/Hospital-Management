...
const Treatment_Details_Controller = {
  ...
  async getAllTreatments(req, res) {
    try {
      ...
      res
        .status(500)
        .json({ error: "Failed to fetch treatments", details: error.message });
    }
  },
  ...
  async getTreatmentByPatientId(req, res) {
    ...
    try {
      const treatment = await Treatment_Details.findAll({
        where: {
      ...
      const treatmentDetails = await Promise.all(
        treatment.map(async (treatment) => {
          const treatmentDetails = await Treatments.findOne({
      ...
      res
        .status(500)
        .json({ error: "Failed to fetch treatment", details: error.message });
    }
  },
  ...
  async addTreatment(req, res) {
    ...
    try {
      ...
      res
        .status(500)
        .json({ error: "Failed to add treatment", details: error.message });
    }
  },
  ...
  async updateTreatment(req, res) {
    ...
    try {
      ...
      res
        .status(500)
        .json({ error: "Failed to update treatment", details: error.message });
    }
  },
  ...
  async deleteTreatment(req, res) {
    ...
    try {
      ...
      res
        .status(500)
        .json({ error: "Failed to delete treatment", details: error.message });
    }
  },
};
