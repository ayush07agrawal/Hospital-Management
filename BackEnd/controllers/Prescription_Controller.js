...
const prescriptionController = {
  addPrescription: async (req, res) => {
    const { patientId, appointmentId, medicines, tests } = req.body;
    ...
    try {
      ...
      const prescriptionId = newPrescription.Prescription_ID;

      if (medicines && medicines.length > 0) {
        for (const med of medicines) {
          await Contains_Med.create({
            Prescription_ID: prescriptionId,
            Medicine_ID: med.medicineId,
            Quantity: med.quantity,
            Number_Of_Days: med.days,
            Times_A_Days: med.timesPerDay,
            After_Food: med.afterFood,
          });
        }
      }
      ...
      if (tests && tests.length > 0) {
        for (const test of tests) {
          await Requires_Test.create({
            Prescription_ID: prescriptionId,
            Test_Name: test.testName,
            Time_Required: test.timeRequired,
            Cost: test.cost,
          });
        }
      }
  ...
  getPrescriptionByName: async (req, res) => {
    try {
      ...
      if (!prescriptionIDs) {
        return res
          .status(404)
          .json({ success: false, message: "Prescriptions not found." });
      }
      ...
      return res
        ...
        .json({
          success: false,
          message: "Failed to fetch prescription by name.",
        });
    }
  },
};
